...
namespace KaizianMod.Items.Accessories
{
    [AutoloadEquip(EquipType.Body)]
    public class NightmareIncantation : ModItem
    {
        public override void SetStaticDefaults()
        {
            Tooltip.SetDefault("Slime becomes friendly." +
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            player.endurance += 0.18f; //Reduces damage take by 18%(?)
            ...
            //If the dash is not active, immediatly return so we don't do any of the logic for it
            if (!mp.DashActive)
                return;

            /*This is where we set the afterimage effect.  You can replace these two lines with whatever you
             * want to happen during the dash. Some examples include:  spawning dust where the player is,
             * adding buffs, making the player immune, etc..
             * Here we take advantage of "player.eocDash" and "player.armorEffectDrawShadowEOCShield" to get
             * the Shield of Cthulhu's afterimage effect */
        ...
        public override void AddRecipes()
        {
            ...
            r.AddIngredient(ItemID.HiveBackpack);
    ...
    public class ExampleDashPlayer : ModPlayer
    {
        ...
        public override void ResetEffects()
        {
            ...
            //Check if the NightmareIncantation is equipped and also check against this priority:
            ...
            //This is the loop used in vanilla to update/check the non-vanity accessories
            for(int i = 3; i < 8 + player.extraAccessorySlots; i++)
            {
                Item item = player.armor[i];

                /*Set the flag for the NightmareIncantation being equipped if we have it equipped OR immediately
                 *return if any of the accessories are one of the higher-priority one*/
                if (item.type == ItemType<NightmareIncantation>())
                    dashAccessoryEquipped = true;
                else if (item.type == ItemID.EoCShield || item.type == ItemID.MasterNinjaGear || item.type == ItemID.Tabi)
                    return;
            }

            /*If we don't have the NightmareIncantation equipped or the player has the Solar armor set equipped,
             * return immediately. Also return if the player is currently on a mount, since dashes on a mount look
             * weird, or if the dash was already activated*/
            if (!dashAccessoryEquipped || player.setSolar || player.mount.Active || DashActive)
                return;

            /*When a directional key is pressed and released, vanilla starts a 15 tick (1/4 second) timer during
             * which a second press activates a dash. If the timers are set to 15, then this is the first press
             * just processed by the vanilla logic.  Otherwise, it's a double-tap*/
